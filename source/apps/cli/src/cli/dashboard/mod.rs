...
pub async fn handle_show_dashboard(database_connection: &DatabaseConnection) {
    ...
    let ingestion_analysis_map: Arc<Mutex<HashMap<i32, IngestionAnalysis>>> = Arc::new(Mutex::new(HashMap::new()));
        ...
        .map_init(
            ...
            |rt, ingestion| {
                ...
                rt.block_on(async move {
                    let ingestion_analysis = analyze_ingestion(analyze_ingestion_from_ingestion(ingestion.clone()).await.unwrap()).await.unwrap();
    ...
    let active_ingestion_analysis = ingestion_analysis_map.into_values()
        .filter(|ingestion| {
    ...
    active_ingestion_analysis.into_iter().for_each(render_active_ingestion_from_ingestion_analysis);
}
