...
async fn get_period_statistics(
    ...
    let rolling_window_start_date = chrono::Local::now().date_naive() - Days::new(rolling_window as u64);
...
impl CommandHandler for GetStatistics {
    async fn handle<'a>(&self, context: AppContext<'a>) -> Result<()> {
        ...
        println!(
            "\n{}\n{}",
            format!("Substance Statistics (Last {} Days)", self.rolling_window).bold().underline().blue(),
