...
export interface StripeModuleConfig
  extends Partial<Stripe.StripeConfig>
  {
	 ...
	 webhookConfig? : {
		 ...
		 *
		 * Note: these decorators must only set metadata that will be read at request time. Decorators like Nest's
		 * `@UsePipes()` or `@UseInterceptors()` wll not work, due to the time at which Nest reads the metadata for
		 * those, but something  that uses `SetMetadata` will be fine, because that metadata is read at request time.
		 */
